    ...
    >
      ...
      >
        ...
        <div className="flex flex-col md:flex-row">
          {/* Left: image */}
          <div className="md:w-[55%] bg-[#f0f4f2] flex-shrink-0">
            {layout.coverImageUrl ? (
              <img
                src={layout.coverImageUrl}
                alt={layout.title}
                className="w-full h-full object-cover aspect-video md:aspect-auto md:h-full min-h-[220px]"
              />
            ) : (
              <div className="w-full aspect-video md:aspect-auto md:h-full min-h-[220px]
          ...
          <div className="flex-1 flex flex-col p-6 gap-4 overflow-y-auto max-h-[90vh] md:max-h-none">
            {/* Event type pill */}
            <span className={`self-start px-2.5 py-0.5 rounded-full text-xs font-semibold ${eventColor}`}>
              {eventLabel}
            </span>
            ...
            <div className="flex items-center gap-4 py-3 border-y border-[#edf0ee]">
              <div className="flex items-center gap-1.5 text-xs text-[#657872]">
                ...
                <span className="font-semibold text-[#314a43]">{layout.forkCount}</span> uses
              </div>
              <div className="flex items-center gap-1.5 text-xs text-[#657872]">
                <Heart size={13} strokeWidth={2} />
                <span className="font-semibold text-[#314a43]">{layout.likeCount}</span> likes
              </div>
              <div className="flex items-center gap-1.5 text-xs text-[#657872]">
                <Bookmark size={13} strokeWidth={2} />
                <span className="font-semibold text-[#314a43]">{layout.saveCount}</span> saved
              </div>
              {layout.estimatedCapacity && (
                <div className="flex items-center gap-1.5 text-xs text-[#657872]">
                  <Users size={13} strokeWidth={2} />
                  <span className="font-semibold text-[#314a43]">{layout.estimatedCapacity}</span> pax
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-full bg-[#d4e3de] flex items-center justify-center
                              text-xs font-bold text-[#5d7f73] flex-shrink-0">
            ...
            <div className="mt-auto pt-2 flex flex-col gap-2">
              ...
              <p className="text-center text-xs text-[#9ca8a3]">
                Creates a private copy in your workspace. The original stays unchanged.
              </p>
            </div>
          </div>
        </div>
