...
import {
  ...
  getPromptClarifications,
} from "@/lib/promptClarification";
...
export default function ProjectModal({
  ...
  const PipelineCard = ({
    ...
    <button
      onClick={onClick}
      className="flex w-full items-center gap-4 rounded-lgborder border-(--sf-border) bg-white p-4 text-left transition hover:border-(--sf-border-strong) hover:shadow-(--sf-shadow)"
    >
      <div className="flex h-11 w-11 flex-0 items-center justify-center rounded-lgbg-(--sf-surface-soft) text-lg text-(--sf-text-muted)">
        ⬢
      </div>

      <div>
        <div className="text-[14px] font-semibold text-(--sf-text)">
          {title}
        </div>
        <div className="text-[12px] text-(--sf-text-muted)">{subtitle}</div>
      </div>
    </button>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 px-6 py-6 backdrop-blur-sm">
      <div className="w-full max-w-xl rounded-xl border border-(--sf-border) bg-white p-6 shadow-(--sf-shadow-md)">
        <div className="mb-5 flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleClose}
              className="flex h-8 w-8 items-center justify-center rounded-full text-2xl leading-none text-(--sf-text-muted) transition hover:bg-(--sf-surface-soft) hover:text-(--sf-text)"
            >
              <X />
            </button>
          </div>
        ...
        {step === "template" && (
          <div>
            ...
            <p className="mt-1 text-[13px] text-(--sf-text-muted)">
            ...
            <input
              ...
              className="mt-4 w-full rounded-lgborder border-(--sf-border-strong) bg-white px-4 py-3 text-[13px] text-(--sf-text) placeholder:text-(--sf-text-faint)"
            />
                ...
                return (
                  <button
                    ...
                    className={`rounded-[10px] border p-4 text-left transition ${
                      active
                        ? "border-(--sf-accent-blue) bg-blue-50"
                        : "border-(--sf-border) bg-white hover:border-(--sf-border-strong)"
                    }`}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-[12px] text-(--sf-text-muted)">
            ...
            <button
              ...
              disabled={loading}
              className="mt-5 rounded-lgbg-[var(--sf-accent-blue)] px-4 py-2 text-[13px] font-medium text-white transition hover:bg-[#1d4ed8] disabled:opacity-50"
            >
        ...
        {step === "upload" && (
          <div>
            ...
            <div className="mt-4 rounded-lgborder border-dashed border-(--sf-border-strong) bg-(--sf-surface-soft) p-5">
              <div className="text-[13px] font-medium text-(--sf-text)">
              ...
              <input
                ...
                className="mt-3 block w-full text-[13px] text-(--sf-text-muted) file:mr-4 file:rounded-md file:border-0 file:bg-(--sf-text) file:px-4 file:py-2 file:text-white"
                onChange={(e) => setFile(e.target.files?.[0] || null)}
              />

              {file ? (
                <div className="mt-3 text-[13px] font-medium text-(--sf-success)">
                  Selected: {file.name}
                </div>
              ) : null}
            </div>
            ...
            <div className="mt-5 flex justify-end">
              <button
                ...
                disabled={loading}
                className="rounded-md bg-[#5d7f73] px-5 py-2.5 text-[13px] font-medium text-white transition hover:bg-[#90bbac] disabled:opacity-60"
              >
        ...
        {step === "prompt" && (
          <div>
            ...
            <textarea
              ...
              className="mt-4 min-h-30 w-full rounded-lgborder border-(--sf-border-strong) bg-white px-4 py-3 text-[13px] text-(--sf-text) placeholder:text-(--sf-text-faint)"
            />

            {prompt.trim() && clarityQuestions.length ? (
              <div className="mt-4 space-y-3 rounded-lgborder border-(--sf-border) bg-[#f7faf8] p-3">
            ...
            <div className="mt-5 flex justify-end">
              <button
                ...
                disabled={loading}
                className="rounded-md bg-[#5d7f73] px-5 py-2.5 text-[13px] font-medium text-white transition hover:bg-[#7ca798] disabled:opacity-60"
              >
                {loading ? "Generating..." : "Generate project"}
              </button>
            </div>
          ...
          <div>
            ...
            <div className="mt-5 flex justify-end">
              <button
                ...
                disabled={loading}
                className="rounded-md bg-[#5d7f73] px-5 py-2.5 text-[13px] font-medium text-white transition hover:bg-[#333333] disabled:opacity-60"
              >
          ...
          <div>
            ...
            <div className="mt-5 flex justify-end">
              <button
                ...
                disabled={loading}
                className="rounded-md bg-(--sf-text) px-5 py-2.5 text-[13px] font-medium text-white transition hover:bg-[#333333] disabled:opacity-60"
              >
        ...
        {error && (
          <div className="mt-4 rounded-lgborder border-red-200 bg-red-50 px-4 py-3 text-[13px] text-red-600">
            {error}
          </div>
        )}
      </div>
