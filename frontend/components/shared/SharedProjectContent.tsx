...
export default function SharedProjectContent() {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-(--sf-bg) px-6">
        <div className="max-w-xl rounded-xl border border-(--sf-border) bg-white p-8 shadow-(--sf-shadow-md">
          ...
          <p className="mt-3 text-[14px] leading-7 text-(--sf-text-muted)">
            {error}
          </p>
          <Link
            href="/"
            className="mt-6 inline-flex rounded-md bg-(--sf-text) px-5 py-3 text-[13px] font-medium text-white hover:bg-[#333333]"
          >
            Back to home
          </Link>
        </div>
  ...
  return (
    <main className="flex min-h-screen flex-col bg-(--sf-bg)">
      <div
        className={`h-13 items-center justify-between border-b border-(--sf-border) bg-white px-6 ${
          presentationMode ? "hidden" : "flex"
        }`}
      >
        <div>
          <div className="text-[15px] font-semibold tracking-[-0.03em] text-(--sf-text)">
            {project.name}
          </div>
        ...
        <Link
          href="/"
          className="rounded-md border border-(--sf-border-strong) px-3 py-1.5 text-[13px] font-medium text-(--sf-text) transition hover:bg-(--sf-surface-soft)"
        >
          Open SpaceForge
        </Link>
      </div>
            ...
            <button
              onClick={() =>
                updateViewerSettings({
                  cameraMode:
                    cameraMode === "walkthrough" ? "orbit" : "walkthrough",
                })
              }
